 ...
 * Click a project name to see its full KPI summary.
 */
...
function StatCard({ label, value, color }: { label: string; value: number | string; color: string }) {
  return (
    <div style={{
      borderRadius: 20, padding: "20px 22px", background: color,
...
function FilterSelect({ label, value, options, onChange }: { label: string; value: string; options: string[]; onChange: (v: string) => void }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
      <label style={{ fontSize: 11, fontWeight: 700, color: "var(--muted)", textTransform: "uppercase", letterSpacing: "0.06em" }}>{label}</label>
      <div style={{ position: "relative" }}>
        <select value={value} onChange={e => onChange(e.target.value)}
          style={{ width: "100%", appearance: "none", borderRadius: 12, border: "1.5px solid var(--border)", padding: "9px 36px 9px 14px", fontSize: 13, fontWeight: 600, color: "var(--text)", background: "var(--surface)", outline: "none", cursor: "pointer", fontFamily: "inherit" }}>
...
export function PortfolioDashboardPage() {
  ...
  // Role-aware base path for project summary navigation
  const basePath =
  ...
  if (loading) return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 16 }}>
        {[1, 2, 3, 4].map(i => <div key={i} style={{ borderRadius: 20, height: 96, background: "var(--border)" }} />)}
      </div>
      <div style={{ borderRadius: 20, height: 72, background: "var(--border)" }} />
      {[1, 2, 3].map(i => <div key={i} style={{ borderRadius: 16, height: 64, background: "rgba(108,99,255,0.06)" }} />)}
    </div>
  );
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      {/* Page title */}
      <div>
        <h1 style={{ fontSize: 28, fontWeight: 900, color: "var(--text)", margin: 0 }}>Portfolio Dashboard</h1>
        ...
        <div style={{ background: "var(--surface)", borderRadius: 20, boxShadow: "var(--shadow)", overflow: "hidden" }}>
          ...
          {filtered.map((p, idx) => (
            <div key={p.id}
              ...
              onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.background = "rgba(108,99,255,0.05)"; }}
              onMouseLeave={e => { (e.currentTarget as HTMLDivElement).style.background = "transparent"; }}
            >
              <div>
                <p style={{ fontWeight: 800, color: "var(--primary)", margin: 0, fontSize: 14 }}>{p.project_name}</p>
                <p style={{ fontSize: 11, color: "var(--muted)", fontFamily: "monospace", margin: "2px 0 0" }}>{p.project_code}</p>
              </div>
              <span style={{ fontSize: 13, color: "var(--muted)" }}>{p.business_unit_name || "—"}</span>
              <span style={{ fontSize: 13, color: "var(--muted)" }}>{p.account_name || "—"}</span>
              <span style={{ fontSize: 13, color: "var(--muted)" }}>{p.kpiPlan?.project_type || "—"}</span>
              <span style={{ fontSize: 13, color: "var(--muted)" }}>{p.kpiPlan?.project_category || "—"}</span>
