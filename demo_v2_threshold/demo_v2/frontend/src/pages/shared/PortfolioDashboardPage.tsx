  ...
  return (
    <div style={{ display: "flex", flexDirection: "column" as const, alignItems: "center", gap: 16 }}>
      <svg viewBox="0 0 128 128" style={{ width: 160, height: 160, filter: "drop-shadow(0 2px 8px rgba(108,99,255,0.15))" }}>
        ...
        <text x={cx} y={cy - 6} textAnchor="middle" fontSize={24} fontWeight="800" fill="#1a1a2e">{total - noData}</text>
        <text x={cx} y={cy + 12} textAnchor="middle" fontSize={9} fill="#6b7280" fontWeight="600">of {total}</text>
...
function StatCard({ label, count, bg, icon }: { label: string; count: number; bg: string; icon: React.ReactNode }) {
  return (
    ...
    onMouseLeave={e => (e.currentTarget as HTMLDivElement).style.transform = "translateY(0)"}>
      ...
      <p style={{ fontSize: 36, fontWeight: 900, color: "#fff", lineHeight: 1, marginTop: 20 }}>{count}</p>
    ...
    <div style={{ display: "flex", flexDirection: "column" as const, gap: 24, fontFamily: "'Inter','Poppins',system-ui,sans-serif" }}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap" as const, alignItems: "center", justifyContent: "space-between", gap: 16 }}>
        <div>
          ...
          <h1 style={{ fontSize: 24, fontWeight: 800, color: "#1a1a2e", margin: 0, letterSpacing: "-0.02em" }}>Portfolio Dashboard</h1>
          <p style={{ fontSize: 13, color: "#6b7280", marginTop: 4 }}>{filteredProjects.length} of {allProjects.length} projects</p>
        </div>
      </div>

      {/* ── Stat cards ── */}
      <div style={{ display: "flex", flexWrap: "wrap" as const, gap: 16 }}>
        <StatCard label="Total Projects" count={ragCounts.total}  bg="linear-gradient(135deg,#7c3aed,#6d28d9)" icon={<svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="#fff" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z"/></svg>} />
        <StatCard label="Green Health"   count={ragCounts.green}  bg="linear-gradient(135deg,#16a34a,#15803d)" icon={<svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="#fff" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>} />
        <StatCard label="Needs Attention" count={ragCounts.amber} bg="linear-gradient(135deg,#d97706,#b45309)" icon={<svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="#fff" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>} />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "280px 1fr", gap: 20 }}>
        ...
        <GlassCard style={{ padding: 0, overflow: "hidden" }}>
          <div style={{ padding: "16px 18px", borderBottom: "1px solid #e8e6ff" }}>
            ...
            <p style={{ fontSize: 13, fontWeight: 700, color: "#1a1a2e", margin: 0 }}>{filteredProjects.length} projects</p>
          </div>
          <div style={{ maxHeight: 520, overflowY: "auto", padding: "10px 12px", display: "flex", flexDirection: "column" as const, gap: 6 }}>
            {filteredProjects.length === 0 && (
              <p style={{ fontSize: 12, color: "#6b7280", textAlign: "center", padding: "32px 0" }}>No projects match filters</p>
            )}
            {filteredProjects.map(p => {
              const rag = p.current_rag;
              const color = rag ? (RAG_COLOR[rag] ?? "#6b7280") : "#e8e6ff";
              ...
              return (
                ...
                onMouseLeave={e => { if (!isSelected) (e.currentTarget as HTMLDivElement).style.background = "#fff"; }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
                    <p style={{ fontSize: 12, fontWeight: 700, color: "#1a1a2e", margin: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" as const, flex: 1 }}>
                      {p.project_name}
                    </p>
                    {rag ? <RagPill rag={rag} size="xs" /> : <span style={{ fontSize: 9, color: "#9ca3af" }}>No data</span>}
                  </div>
                  <p style={{ fontSize: 10, color: "#6b7280", margin: "3px 0 0" }}>{p.business_unit_name || "—"} · {p.account_name || "—"}</p>
                </div>
              );
            })}
          </div>
        ...
        <div style={{ display: "flex", flexDirection: "column" as const, gap: 18 }}>
          ...
          {!selectedId && (
            <GlassCard style={{ padding: "56px 24px", textAlign: "center" }}>
              ...
              <p style={{ color: "#6b7280" }}>Select a project to view its KPI summary</p>
            </GlassCard>
          )}
            ...
            <>
                ...
                return (
                  <div style={{
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <p style={{ fontSize: 20, fontWeight: 900, color: "#1a1a2e", margin: 0 }}>{rag}</p>
                    </div>
                    <div style={{ textAlign: "right" as const }}>
                      <p style={{ fontSize: 13, fontWeight: 700, color: "#1a1a2e", marginBottom: 4 }}>{selectedProject?.project_name}</p>
                      <RagPill rag={rag} />
                    </div>
                  </div>
                );
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(280px,1fr))", gap: 18 }}>
                ...
                <GlassCard style={{ padding: 24 }} className="kpi-card-hover">
                  ...
                  <h2 style={{ fontSize: 14, fontWeight: 700, color: "#1a1a2e", margin: "0 0 16px" }}>KPI Health</h2>
                  {selectedSummary ? (
                    <RagDonut
                    ...
                    />
                  ) : <p style={{ color: "#6b7280", fontSize: 12 }}>No KPI data yet</p>}
                </GlassCard>

                {/* Dimensions */}
                <GlassCard style={{ padding: 24 }} className="kpi-card-hover">
                  <SectionLabel>Breakdown</SectionLabel>
                  <h2 style={{ fontSize: 14, fontWeight: 700, color: "#1a1a2e", margin: "0 0 4px" }}>Dimension Status</h2>
                  <p style={{ fontSize: 11, color: "#6b7280", marginBottom: 14 }}>Click to expand</p>
                  {!selectedSummary || Object.keys(selectedSummary.category_rag).length === 0 ? (
                    ...
                    <div style={{ display: "flex", flexDirection: "column" as const, gap: 8 }}>
                      {Object.entries(selectedSummary.category_rag).map(([cat, rag]) => {
                        ...
                        const color = RAG_COLOR[rag as string] ?? "#6b7280";
                          ...
                          <div key={cat} style={{ borderRadius: 12, overflow: "hidden", background: `${color}08`, border: `1px solid ${color}22` }}>
                            <button type="button" onClick={() => setExpandedDimension(isOpen ? null : cat)}
                              ...
                              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                                ...
                                <span style={{ fontSize: 10, color: "#6b7280" }}>{dimMetrics.length} metric{dimMetrics.length !== 1 ? "s" : ""}</span>
                              </div>
                                ...
                                {dimMetrics.filter(m => m.rag_status).length === 0
                                  ...
                                  : dimMetrics.filter(m => m.rag_status).map(m => (
                                    <div key={m.plan_metric_id} style={{ padding: "9px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, borderBottom: "1px solid #e8e6ff" }}>
                                      <p style={{ fontSize: 11, fontWeight: 600, color: "#1a1a2e", flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" as const }}>{m.metric_name}</p>
              ...
              {selectedSummary && selectedSummary.metrics.length > 0 && (
                <div>
                  <div style={{ marginBottom: 14 }}>
                    ...
                    <h2 style={{ fontSize: 14, fontWeight: 700, color: "#1a1a2e", margin: 0 }}>All Metrics</h2>
                  </div>
                    ...
                    {selectedSummary.metrics.map(m => {
                      ...
                      return (
                        <div key={m.plan_metric_id} className="kpi-card-hover" style={{
                          ...
                          background: "#ffffff", border: `1px solid ${color}28`,
                          ...
                          <p style={{ fontSize: 12, fontWeight: 700, color: "#1a1a2e", lineHeight: 1.3, marginBottom: 10,
                          ...
                          <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between" }}>
                            <div>
                              ...
                              <p style={{ fontSize: 20, fontWeight: 900, color: "#1a1a2e", lineHeight: 1 }}>
              ...
              {!selectedSummary && !loadingSummary && (
                <GlassCard style={{ padding: "48px 24px", textAlign: "center" }}>
                  ...
                  <p style={{ color: "#6b7280", fontSize: 14 }}>No KPI plan found for {selectedProject?.project_name}.</p>
                </GlassCard>
              )}
            </>
          )}
