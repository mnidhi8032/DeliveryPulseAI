/**
 * Delivery Head — Project Health Timeline (light theme)
 */
...
function HealthChart({ events }: { events: SubmissionTimelineEvent[] }) {
  ...
  const P = { t: 20, r: 24, b: 36, l: 44 };
  const cW = W - P.l - P.r; const cH = H - P.t - P.b;
  const toX = (i: number) => P.l + i * (cW / (scored.length - 1));
  const toY = (v: number) => P.t + cH - (v / 100) * cH;
  const pts  = scored.map((e, i) => `${toX(i)},${toY(Number(e.overall_score))}`).join(" ");
  const area = [`${toX(0)},${P.t + cH}`, ...scored.map((e, i) => `${toX(i)},${toY(Number(e.overall_score))}`), `${toX(scored.length - 1)},${P.t + cH}`].join(" ");

  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ maxHeight: 200 }}>
      <defs>
        <linearGradient id="dhAreaGrad" x1="0" y1="0" x2="0" y2="1">
      ...
      <rect x={P.l} y={toY(80)}  width={cW} height={toY(50) - toY(80)}  fill="#f59e0b08" />
      <rect x={P.l} y={toY(50)}  width={cW} height={toY(0)  - toY(50)}  fill="#f43f5e08" />
      <polygon points={area} fill="url(#dhAreaGrad)" opacity={0.7} />
      ...
      {scored.map((e, i) => {
        ...
        return (
          <g key={i}>
            <circle cx={toX(i)} cy={toY(s)} r={5} fill={RAG_COLOR(s)} stroke="white" strokeWidth={2} />
            <text x={toX(i)} y={toY(s) - 9} textAnchor="middle" fontSize={9} fontWeight="bold" fill={RAG_COLOR(s)}>{s.toFixed(1)}</text>
          </g>
        );
      })}
      {scored.map((e, i) => (
...
function RagPill({ rag }: { rag: string }) {
  ...
  return (
    <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-bold border ${map[rag] ?? "bg-slate-100 text-slate-500 border-slate-200"}`}>
      {rag}
    </span>
  );
}

export function DHProjectTimelinePage() {
  ...
  useEffect(() => {
    ...
    Promise.all([getProject(projectId), getSubmissionTimeline(projectId)])
      .then(([proj, evts]) => { setProject(proj); setTimeline(evts); })
      .catch((e: any) => {
  ...
  if (error || !project) return <p className="text-sm text-red-600">{error || "Project not found."}</p>;
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      ...
      {/* back */}
      <button onClick={() => navigate(-1)} className="flex items-center gap-1.5 text-sm text-slate-500 hover:text-slate-800 transition-colors cursor-pointer">
      ...
      {/* hero banner — keeps colour gradient */}
      <div className={`relative overflow-hidden rounded-xl bg-gradient-to-r ${heroBg} p-6 text-white shadow-lg`}>
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            {latestRag && latest && (
              <div className="text-right">
                ...
                <p className="mt-1 text-3xl font-extrabold">{latest.overall_score !== null ? Number(latest.overall_score).toFixed(1) : "—"}</p>
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-4 text-xs text-slate-500">
            <span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-emerald-500" /> Green ≥ 80</span>
            <span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-amber-500" /> Amber 50–79</span>
            <span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-rose-500" /> Red &lt; 50</span>
          </div>
        </div>
      ...
      {scored.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            ...
            { label: "Best Score",         value: Math.max(...scored.map(e => Number(e.overall_score))).toFixed(1) },
            { label: "Latest Score",       value: latest ? Number(latest.overall_score).toFixed(1) : "—" },
          ].map(s => (
      ...
      <div className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-slate-100 text-sm text-left text-slate-600">
            ...
            <tbody className="divide-y divide-slate-100 bg-white">
              {timeline.length === 0 ? (
                <tr><td colSpan={6} className="px-6 py-8 text-center text-slate-400">No submissions found.</td></tr>
              ) : timeline.map(event => (
                <tr key={event.submission_id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-6 py-4 font-bold text-slate-900">
                    {event.overall_score !== null ? Number(event.overall_score).toFixed(1) : <span className="text-slate-400 font-normal">—</span>}
                  </td>
                  <td className="px-6 py-4">
                    {event.rag_status ? <RagPill rag={event.rag_status} /> : <span className="text-slate-400">—</span>}
                  </td>
                  <td className="px-6 py-4">
                    {event.trend === "improving" && <span className="text-emerald-600 font-semibold text-xs flex items-center gap-1">↑ Improving</span>}
                    {event.trend === "declining" && <span className="text-rose-600 font-semibold text-xs flex items-center gap-1">↓ Declining</span>}
                    {event.trend === "stable"    && <span className="text-slate-500 font-semibold text-xs">→ Stable</span>}
                  ...
                  <td className="px-6 py-4">
                    ...
                    {event.actor_role && <div className="text-xs text-slate-400 mt-0.5">{event.actor_role}</div>}
                  </td>
                </tr>
              ))}
            </tbody>
