/**
 * Delivery Head — Project KPI Summary (dark theme)
 * Full read-only replica of PM KPI Summary, styled to match the DH dark canvas.
 */
...
function RagDonut({ green, amber, red, noData, categoryRag }: {
  ...
  const slices = [
    ...
    { rag: null,     value: noData, color: "#334155" },
  ];
  let offset = 0;
    ...
    <div className="flex flex-col items-center gap-3 w-full">
      <div className="relative">
        <svg viewBox="0 0 128 128" className="w-36 h-36">
          ...
          <text x={cx} y={cy - 4} textAnchor="middle" fontSize={22} fontWeight="bold" fill="#f1f5f9">{total - noData}</text>
          <text x={cx} y={cy + 14} textAnchor="middle" fontSize={9} fill="#64748b">of {total}</text>
        </svg>
        {hovered && hoveredDims.length > 0 && (
          <div className="absolute left-full top-1/2 -translate-y-1/2 ml-3 z-20 rounded-xl border border-white/10 bg-[#1e1e2e] shadow-2xl p-3 min-w-[160px] pointer-events-none">
      ...
      <div className="flex flex-wrap gap-3 justify-center text-xs">
        {[["GREEN", green, "#10b981"], ["AMBER", amber, "#f59e0b"], ["RED", red, "#f43f5e"], ["No Data", noData, "#475569"]].map(([l, v, c]) => (
          <div key={l as string} className="flex items-center gap-1.5">
            ...
            <span className="text-slate-400">{l}: <b className="text-slate-200">{v}</b></span>
          </div>
        ))}
      </div>
    </div>
...
// ── Metric trend chart (dark) ────────────────────────────────────────────────
function MetricTrendChart({ metric }: { metric: KpiSummaryMetric }) {
  const history = metric.history ?? [];
  if (history.filter(h => h.actual_value != null).length < 1)
    return <div className="flex items-center justify-center h-40 text-xs text-slate-500 bg-white/5 rounded-xl border border-white/5">Not enough data to show trend</div>;
    ...
    <div className="space-y-2">
      ...
      <div className="rounded-xl border border-white/5 bg-white/5 overflow-x-auto">
        <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ minWidth: "340px", height: "160px" }}>
          {ticks.map((t, i) => (
            <g key={i}>
              <line x1={padL} y1={yP(t)} x2={W - padR} y2={yP(t)} stroke="#ffffff08" strokeWidth="1" strokeDasharray="3,3" />
              <text x={padL - 4} y={yP(t) + 4} textAnchor="end" fontSize="8" fill="#475569">{t % 1 === 0 ? t.toFixed(0) : t.toFixed(1)}</text>
            </g>
          ))}
          {history.map((h, i) => <text key={i} x={xP(i)} y={H - 6} textAnchor="middle" fontSize="8" fill="#475569">{(h.frequency_name || "").substring(0, 8)}</text>)}
          ...
          {history.map((h, i) => h.actual_value != null && (
            <circle key={i} cx={xP(i)} cy={yP(Number(h.actual_value))} r="4"
              fill={h.rag_status ? RAG_HEX[h.rag_status] || "#3b82f6" : "#3b82f6"} stroke="#13131f" strokeWidth="1.5">
      ...
      <div className="overflow-x-auto rounded-xl border border-white/5">
        <table className="min-w-full text-[10px]">
          <thead className="bg-white/5 text-slate-500 font-semibold">
            <tr>
              <th className="px-3 py-1.5 text-left">Period</th>
              <th className="px-3 py-1.5 text-right text-blue-400">Actual</th>
              <th className="px-3 py-1.5 text-right text-emerald-400">Target</th>
              <th className="px-3 py-1.5 text-right text-amber-400">LSL</th>
              <th className="px-3 py-1.5 text-right text-rose-400">USL</th>
              <th className="px-3 py-1.5 text-center">RAG</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-white/5">
            {[...history].reverse().map((h, i) => (
              <tr key={i} className={i === 0 ? "bg-violet-500/10" : "hover:bg-white/5"}>
                <td className="px-3 py-1.5 font-medium text-slate-300">
                  {h.frequency_name || "--"}
                  {i === 0 && <span className="ml-1.5 text-[8px] bg-violet-500/20 text-violet-300 rounded px-1 font-bold">Latest</span>}
                </td>
                <td className="px-3 py-1.5 text-right font-bold text-white">{h.actual_value != null ? Number(h.actual_value).toFixed(2) : "--"}</td>
                <td className="px-3 py-1.5 text-right text-slate-400">{h.target != null ? Number(h.target).toFixed(2) : "--"}</td>
                <td className="px-3 py-1.5 text-right text-slate-400">{h.lsl != null ? Number(h.lsl).toFixed(2) : "--"}</td>
                <td className="px-3 py-1.5 text-right text-slate-400">{h.usl != null ? Number(h.usl).toFixed(2) : "--"}</td>
                <td className="px-3 py-1.5 text-center">
                  {h.rag_status
                    ? <span className={`rounded-full px-2 py-0.5 text-[8px] font-bold border ${RAG_PILL[h.rag_status] ?? "bg-white/5 text-slate-400 border-white/10"}`}>{h.rag_status}</span>
...
export function DHProjectSummaryPage() {
  ...
  if (loading) return (
    <div className="space-y-5 animate-pulse">
      ...
      <div className="grid grid-cols-4 gap-4">{[...Array(4)].map((_, i) => <div key={i} className="h-20 rounded-2xl bg-slate-700" />)}</div>
      <div className="grid md:grid-cols-2 gap-5">{[1,2].map(i => <div key={i} className="h-64 rounded-2xl bg-slate-700" />)}</div>
    </div>
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2 text-xs text-slate-500">
        <Link to="/delivery-head" className="hover:text-slate-300 transition-colors">Dashboard</Link>
        <span>/</span>
        <Link to="/delivery-head/projects" className="hover:text-slate-300 transition-colors">Projects</Link>
        <span>/</span>
        <span className="text-slate-300 font-medium">{project?.project_name ?? "Project"}</span>
      </div>
      ...
      <div className="rounded-2xl bg-[#1e1e2e] border border-white/5 p-5 shadow-lg">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Project · Read-only</p>
            <h1 className="mt-0.5 text-2xl font-black text-white">{project?.project_name}</h1>
            <p className="font-mono text-xs text-slate-500 mt-0.5">{project?.project_code}</p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            {project?.current_rag && (
              <span className={`rounded-lg px-3 py-1.5 text-xs font-bold border ${RAG_PILL[project.current_rag] ?? "bg-white/5 text-slate-400 border-white/10"}`}>
                ● {project.current_rag}
              </span>
            )}
            <Link to={`/delivery-head/projects/${projectId}/timeline`}
              className="rounded-lg border border-white/10 bg-white/5 px-3 py-1.5 text-xs font-semibold text-slate-300 hover:bg-white/10 transition-colors">
              Health Timeline →
            </Link>
          </div>
        </div>
      ...
      {noKpiPlan && (
        <div className="rounded-2xl border border-dashed border-white/10 bg-white/5 p-10 text-center">
          <p className="text-sm font-semibold text-slate-400">No KPI Plan found for this project.</p>
          <p className="text-xs text-slate-500 mt-1">The PM hasn't created a KPI plan yet.</p>
        </div>
      )}
      ...
      {summary && (
        <>
          {/* overall RAG banner */}
          {rag && (
            <div className="rounded-2xl border border-white/5 bg-[#1e1e2e] p-5 flex items-center gap-4 shadow-lg">
              <div className="w-14 h-14 rounded-xl flex items-center justify-center font-black text-white text-2xl shrink-0"
                style={{ background: RAG_HEX[rag] ?? "#64748b" }}>
                {rag[0]}
              </div>
              <div>
                <p className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Overall Project Health</p>
                <p className="text-2xl font-black mt-0.5" style={{ color: RAG_HEX[rag] ?? "#f1f5f9" }}>{rag}</p>
              </div>
              <div className="ml-auto grid grid-cols-4 gap-3 text-center">
                {[
                  { label: "Total",   v: summary.total_metrics,  c: "text-white" },
                  { label: "Green",   v: summary.green_count,    c: "text-emerald-400" },
                  { label: "Amber",   v: summary.amber_count,    c: "text-amber-400" },
                  { label: "Red",     v: summary.red_count,      c: "text-rose-400" },
                ].map(s => (
                  <div key={s.label}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {/* donut */}
            <div className="rounded-2xl bg-[#1e1e2e] border border-white/5 p-5 shadow-lg flex flex-col items-center gap-3 relative overflow-visible">
              <div className="self-start">
                <h2 className="text-sm font-bold text-white">KPI Health Distribution</h2>
                <p className="text-[10px] text-slate-500 mt-0.5">Hover a slice to see dimensions</p>
              </div>
            ...
            {/* dimension accordion */}
            <div className="rounded-2xl bg-[#1e1e2e] border border-white/5 p-5 shadow-lg">
              <h2 className="text-sm font-bold text-white mb-1">Dimension Status</h2>
              <p className="text-[10px] text-slate-500 mb-4">Click a dimension to expand its metrics</p>
                ...
                : (
                  <div className="space-y-2">
                    {Object.entries(summary.category_rag).map(([cat, r]) => {
                      ...
                      return (
                        <div key={cat} className={`rounded-xl border overflow-hidden ${RAG_DIM_BG[r] ?? "bg-white/5 border-white/10"}`}>
                          <button type="button" onClick={() => setExpandedDim(isOpen ? null : cat)}
                            ...
                            <div className="flex items-center gap-2">
                              <span className={`rounded-full px-2 py-0.5 text-[9px] font-bold border ${RAG_PILL[r] ?? "bg-white/5 text-slate-400 border-white/10"}`}>{r}</span>
                              <span className="text-xs font-semibold text-slate-200">{cat}</span>
                              <span className="text-[9px] text-slate-500">{dims.length} metric{dims.length !== 1 ? "s" : ""}</span>
                            </div>
                            <svg className={`h-3.5 w-3.5 text-slate-500 transition-transform ${isOpen ? "rotate-180" : ""}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                          ...
                          {isOpen && (
                            <div className="border-t border-white/5 bg-white/5 divide-y divide-white/5">
                              {dims.length === 0
                                ? <p className="px-4 py-3 text-xs text-slate-500">No data entered yet.</p>
                                : dims.map(m => (
                                  <div key={m.plan_metric_id} className="px-4 py-2.5 flex items-center justify-between gap-3">
                                    <p className="text-xs font-semibold text-slate-300 truncate flex-1">{m.metric_name}</p>
                                    <span className={`rounded-full px-2.5 py-0.5 text-[9px] font-bold border shrink-0 ${RAG_PILL[m.rag_status!] ?? "bg-white/5 text-slate-400 border-white/10"}`}>{m.rag_status}</span>
                                  </div>
                                ))
                              }
                            </div>
          ...
          {summary.metrics.length > 0 && (
            <div>
              <h2 className="text-sm font-bold text-white mb-3">All Metrics</h2>
                  ...
                  return (
                    <div key={m.plan_metric_id} className={isExp ? "col-span-full" : ""}>
                      <div onClick={() => setExpandedMetric(isExp ? null : m.plan_metric_id)}
                        className={`rounded-2xl border bg-[#1e1e2e] shadow-lg cursor-pointer hover:bg-[#252538] transition-all ${
                          isExp ? "border-violet-500/40 ring-1 ring-violet-500/20"
                          : r ? RAG_CARD_BORDER[r] : "border-white/5"
                        }`}>
                        {/* card header */}
                        <div className="p-4">
                          <div className="flex items-start justify-between gap-1 mb-2">
                            <div className="min-w-0">
                              <p className="text-[9px] font-bold uppercase tracking-widest text-slate-500 truncate">{m.metric_category}</p>
                              <p className="text-xs font-bold text-white line-clamp-2 mt-0.5">{m.metric_name}</p>
                            </div>
                            <span className="w-2.5 h-2.5 rounded-full shrink-0 mt-0.5" style={{ background: r ? RAG_HEX[r] : "#334155" }} />
                          </div>
                          <div className="flex items-end justify-between gap-2">
                            <div>
                              ...
                              <p className="text-xl font-black text-white leading-none">
                                ...
                                {m.uom && <span className="text-[9px] font-normal text-slate-500 ml-0.5">{m.uom}</span>}
                              </p>
                            </div>
                            <div className="flex items-center gap-1.5">
                              {r && <span className={`rounded-full px-1.5 py-0.5 text-[9px] font-bold border ${RAG_PILL[r]}`}>{r}</span>}
                              <span className="text-[9px] text-violet-400">{isExp ? "collapse" : "trend ↓"}</span>
                            </div>
                          </div>
                        </div>
                        {/* expanded trend */}
                        {isExp && (
                          <div className="border-t border-white/5 p-4" onClick={e => e.stopPropagation()}>
                            <div className="flex items-center justify-between mb-3">
                              <div>
                                <p className="text-sm font-bold text-white">{m.metric_name}</p>
                                <p className="text-[10px] text-slate-500">{m.metric_category} · {m.intent} · {m.uom}</p>
                              </div>
                              <button onClick={() => setExpandedMetric(null)} className="text-xs text-slate-500 hover:text-slate-300 cursor-pointer font-bold">[×]</button>
                            </div>
          ...
          {summary.metrics.length === 0 && (
            <div className="rounded-2xl border border-dashed border-white/10 bg-white/5 p-10 text-center">
              ...
              <p className="text-xs text-slate-500 mt-1">The PM needs to enter measurements in the KPI Data Entry sheet.</p>
            </div>
          )}
        </>
      )}
